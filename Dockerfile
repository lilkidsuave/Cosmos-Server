# syntax=docker/dockerfile:1

FROM debian:11

EXPOSE 443 80

VOLUME /config

WORKDIR /app

ENV PATH=$PATH:/usr/local/go/bin

RUN apt-get update && apt-get install -y ca-certificates openssl fdisk mergerfs snapraid && \
    apt-get install -y --no-install-recommends  wget curl && \
    apt-get install -y --no-install-recommends nodejs && \
    ./usr/bin/wget https://golang.org/dl/go1.21.8.linux-amd64.tar.gz && \
    tar -C /usr/local -xzf go1.21.8.linux-amd64.tar.gz && \
    rm go1.21.8.linux-amd64.tar.gz && \
    ./usr/bin/curl -fsSL https://deb.nodesource.com/setup_18.x | bash - && \
    apt-get install -y nodejs && \
    apt-get remove -y wget curl && \
    apt-get autoremove -y && \
    ./usr/bin/wget https://github.com/slackhq/nebula/releases/download/v1.8.2/nebula-linux-amd64.tar.gz &&\
    tar -xzvf nebula-linux-amd64.tar.gz && \
    rm nebula-linux-amd64.tar.gz

# Copy Go modules and download them, copy npm dependencies and install them
COPY go.mod ./
COPY go.sum ./
RUN go mod download

COPY package.json ./
COPY package-lock.json ./
RUN npm install

COPY . .
RUN npm run client-build && \
    chmod +x build.sh && \
    ./build.sh && \
    rm -rf /usr/local/go \
           /tmp/* \
           /var/lib/apt/lists/* \
           /var/tmp/*

# Start Avahi daemon and run the application
CMD service avahi-daemon start && ./cosmos
